...
const UploadPage: React.FC = () => {
  ...
  const [keepPolling, setKeepPolling] = useState(true); // Controls polling
  ...
  useEffect(() => {
    ...
    if (keepPolling) {
      intervalId = setInterval(loadFiles, 5000); // Poll every 5 seconds
    }
  ...
  return (
    <Box sx={{ textAlign: "left", p: 4 }}>
      ...
      {/* Upload Modal */}
      <UploadModal open={openModal} handleClose={() => setOpenModal(false)} refreshFiles={loadFiles} setKeepPolling={setKeepPolling} />
    </Box>
  );
};
